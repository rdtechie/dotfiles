    ...
    for dns_ip in $dns_servers
        ...
        if not echo $dns_ip | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$'
            if echo $dns_ip | grep -q ':'
                # IPv6 address
                set hostname (host $dns_ip 2>/dev/null | grep 'domain name pointer' | awk '{print $NF}' | sed 's/\.$//')
                if test -n "$hostname"
                    echo "$dns_ip → $hostname"
                else
        ...
        if test -z "$hostname"
            set hostname (dig -x $dns_ip +short 2>/dev/null | head -n 1 | sed 's/\.$//')
        end
